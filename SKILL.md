...
2. 如果用户给了文件路径，先运行 `scripts/check_article_style.py <markdown-file>`。
   - 脚本只用于定位疑似段落和固定结构问题。
...
8. 再运行一次 `scripts/check_article_style.py <markdown-file>`。剩余提示要么继续修，要么说明为什么保留。
...
- `scripts/check_article_style.py <markdown-file>`：辅助定位固定句式、结构不一致、格式问题和疑似 AI 味段落；不会自动改写。
